...
-- customers — dimension chậm thay đổi
...
-- accounts — semi-dimension, balance update liên tục
...
-- REPLICA IDENTITY FULL: Debezium sẽ có before-image đầy đủ khi UPDATE/DELETE.
-- Cost: WAL phình to hơn vì ghi cả row cũ. Chấp nhận vì balance cần audit.
ALTER TABLE accounts REPLICA IDENTITY FULL;

-- transactions — fact append-only, throughput cao
...
-- ---------------------------------------------------------------------
-- transfers — fact có lifecycle, lý tưởng cho test CEP/state machine
-- ---------------------------------------------------------------------
