...
> ⚠️ **`clickhouse/init` KHÔNG được mount.** Khác với `postgres/init`, schema ClickHouse **không** tự
> chạy. Xem [`../guide/clickhouse-grafana.md`](../guide/clickhouse-grafana.md) §1.

> ⚠️ **Bẫy `jvm.config` trên Windows.** `./trino/etc/jvm.config` được mount **kiểu file**. Nếu file
