...
> Phần audit "metadata sprawl" bên dưới giữ làm **bối cảnh lịch sử**; các mục ~~gạch~~ là sprawl đã đóng.
...
Cột "Hiện tại" = trạng thái **2026-07-20** (audit gốc 15/07 hầu hết chưa đạt; các ô đánh [x] là kết quả Pha 1–7).

| Khía cạnh | Hiện tại (20/07) | Mục tiêu production |
|---|---|---|
| Nguồn sự thật schema | [x] 1 registry `metadata/` (dataset + connection) | [x] 1 registry / data contract |
| Thêm cột/bảng | [x] Sửa 1 contract → `cli write` sinh tự động | [x] Sửa 1 contract → sinh tự động |
| Triển khai artifact | [x] Generator + deployer idempotent (connectors/spark/flink/OM); [~] CI apply+rollback | [x] Generator + CI/CD |
| Kiểm soát schema evolution | [x] `cli compat` gate BACKWARD ở PR ([ADR-0030](../decisions/0030-ci-plan-compat-gate.md)) | [x] Compatibility gate |
| Lineage / catalog | [x] OpenMetadata + lineage cột Flink/Spark ([ADR-0026](../decisions/0026-lineage-catalog-from-metadata.md)/[27](../decisions/0027-openmetadata-catalog.md)/[28](../decisions/0028-spark-column-lineage-sqlglot.md)) | [x] Tự động (DataHub/OpenMetadata) |
| Data quality | [x] `verifiers/quality` — not_null/unique/range/accepted_values trên dữ liệu thật ([ADR-0033](../decisions/0033-data-quality-gate.md)) | [x] Rule trong metadata + gate |
| Orchestration | [x] Airflow DAG sinh từ deps ([ADR-0031](../decisions/0031-airflow-dag-from-metadata.md)) | [x] DAG sinh từ phụ thuộc dataset |
| Migration schema | [x] CH runner versioned ([ADR-0032](../decisions/0032-versioned-migration-clickhouse.md)); Iceberg native ([ADR-0036](../decisions/0036-iceberg-native-evolution.md)) | [x] Không init-once |
| Rollback | [x] `connectors --ref` áp lại desired state từ git ref ([ADR-0034](../decisions/0034-rollback-via-git-ref.md)) | [x] Rollback được |
| Federation (query chéo nguồn) | [x] Trino 3 nguồn verify runtime ([ADR-0025](../decisions/0025-connection-registry-trino-catalog.md)) | [x] Trino/engine liên thông |
| Quản lý secrets | Chưa — `.env` local, đã gitignore, không có manager | [x] Vault/SOPS + `secret_ref` |
| RBAC / audit | [x] CODEOWNERS theo vùng + owner contract + audit Git/lineage ([ADR-0035](../decisions/0035-rbac-codeowners.md)) | [x] Ai sửa contract nào + dấu vết |
| HA / chịu lỗi | [ ] Single node toàn bộ | Ngoài phạm vi metadata (nhưng cần) |
...
| 6 | [x] ~~Schema Kafka source `ROW<...>`~~ | **Đã sinh** cho cả metric runner lẫn fraud runner (từ cột contract, [ADR-0023](../decisions/0023-flink-metric-runner-declarative.md)); `lane1_dashboard.py` + `lane3_fraud_detection.py` đã xoá | ~~Sửa cột phải sửa nhiều chỗ~~ — ROW không còn viết tay ở đâu |
| 7 | `group.id` mỗi job | mỗi file Flink | Trùng `group.id` → tranh offset |
| 8 | [x] ~~Schema metric đầu ra (Flink sink)~~ | **Đã sinh** — `flink_sql.py` sinh sink DDL từ **cùng cột contract** với DDL ClickHouse ([ADR-0023](../decisions/0023-flink-metric-runner-declarative.md)); `lane1_dashboard.py` đã xoá | ~~Lệch thủ công với ClickHouse~~ — nay cùng nguồn, không thể lệch |
...
| 11 | [x] ~~Cột tổng hợp Gold~~ | **Đã khai** trong 3 batch spec `metadata/pipelines/batch/gold_*.yaml` (SQL + output columns), chạy bằng `medallion_runner` ([ADR-0024](../decisions/0024-spark-medallion-runner-sql.md)); parity 339/100/310 = job cũ → `build_gold_layer.py` đã xoá | ~~Không ai biết Gold phụ thuộc cột nào~~ — nay khai tường minh |
| 12 | Danh sách DLQ topic | [`dlq-processor/dlq_processor.py`](../../dlq-processor/dlq_processor.py) | Thêm connector phải nhớ thêm DLQ |
| 13 | [x] ~~Catalog nguồn~~ | **Đã sinh** từ connection registry `metadata/connections/*.yaml` ([ADR-0025](../decisions/0025-connection-registry-trino-catalog.md)); 3 catalog khớp byte-exact | ~~Cấu hình rời rạc~~ — nay 1 nguồn |
...
| 2 | [x] ~~**Bucket lake không được tạo**~~ | **Đã xử lý** 2026-07-18 — `minio-init` nay tạo cả `data-lake-{bronze,silver,gold,iceberg}` | S3 sink chạy được ngay khi `up`; không phải tạo tay. |
...
| 4b | [x] ~~**`metrics.notification_events` không tồn tại**~~ | **Đã tạo** qua migration `migrations/clickhouse/0001_notification_events.sql` ([ADR-0032](../decisions/0032-versioned-migration-clickhouse.md)) | Bảng tồn tại; fraud-notifier ghi được. |
...
| 6 | [x] ~~**Print sink còn trong job fraud**~~ | **Đã bỏ** — `fraud_runner.py` (thay `lane3_fraud_detection.py`) không còn `ds.print` ([ADR-0023](../decisions/0023-flink-metric-runner-declarative.md)) | ~~In mọi transaction ra log ở 150 RPS~~ |
