...
| 6 | 🟡 Schema Kafka source `ROW<...>` | Nay **sinh** cho metric runner (từ cột tham chiếu, [ADR-0023](../decisions/0023-flink-metric-runner-declarative.md)); chỉ còn `lane3_fraud_detection.py` viết tay | Giảm mạnh; hết hẳn khi lane3 tham số hoá |
...
Nay chỉ còn **1 file viết tay** khối này:

```text
lane3_fraud_detection  : 'topic' = 'bankdb.public.transactions'   (còn viết tay)
metric_runner.py       : ROW sinh từ cột contract tham chiếu       (đã sinh)
```

*(Đã xoá: `lane1_{timeseries,kpi,breakdown,topn}.py` (print-sink cũ) và `lane1_dashboard.py` (đã sinh).)*
