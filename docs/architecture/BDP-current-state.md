...
| Data quality | ✅ `verifiers/quality` — not_null/unique/range/accepted_values trên dữ liệu thật ([ADR-0033](../decisions/0033-data-quality-gate.md)) | ✅ Rule trong metadata + gate |
| Orchestration | ✅ Airflow DAG sinh từ deps ([ADR-0031](../decisions/0031-airflow-dag-from-metadata.md)) | ✅ DAG sinh từ phụ thuộc dataset |
| Migration schema | ✅ CH runner versioned ([ADR-0032](../decisions/0032-versioned-migration-clickhouse.md)); Iceberg native ([ADR-0036](../decisions/0036-iceberg-native-evolution.md)) | ✅ Không init-once |
| Rollback | ✅ `connectors --ref` áp lại desired state từ git ref ([ADR-0034](../decisions/0034-rollback-via-git-ref.md)) | ✅ Rollback được |
...
| RBAC / audit | ✅ CODEOWNERS theo vùng + owner contract + audit Git/lineage ([ADR-0035](../decisions/0035-rbac-codeowners.md)) | ✅ Ai sửa contract nào + dấu vết |
...
| 13 | ✅ ~~Catalog nguồn~~ | **ĐÃ SINH** từ connection registry `metadata/connections/*.yaml` ([ADR-0025](../decisions/0025-connection-registry-trino-catalog.md)); 3 catalog khớp byte-exact | ~~Cấu hình rời rạc~~ — nay 1 nguồn |
...
| 9 | ✅ ~~**Không orchestration**~~ | **DAG sinh từ deps** ([ADR-0031](../decisions/0031-airflow-dag-from-metadata.md)): silver→gold/iceberg, có lịch/retry/SLA; **boot Airflow thật, DAG load OK** (DagBag, 0 import error). Còn: chạy task spark-submit e2e + backfill. |
| 10 | ✅ ~~**Không lineage / data quality gate**~~ | **Lineage + catalog** (OpenMetadata, lineage cột — ADR-0026/27/28) + **data quality gate** (`verifiers/quality`, not_null/unique/range/accepted_values trên dữ liệu thật — [ADR-0033](../decisions/0033-data-quality-gate.md)). |
