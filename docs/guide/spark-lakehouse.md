...
## 3. Chạy medallion — một lệnh deployer

Nay không submit từng job tay nữa. **Deployer** sinh config từ batch spec
(`metadata/pipelines/batch/*.yaml`) rồi spark-submit `medallion_runner.py` theo **đúng thứ tự phụ thuộc**
(silver → 3 gold → iceberg) — mô hình dbt, [ADR-0024](../decisions/0024-spark-medallion-runner-sql.md):

```bash
python -m dataplatform.deployers.spark_batch plan     # xem 5 job + thứ tự stage
python -m dataplatform.deployers.spark_batch apply    # chạy tất cả
```

Dưới đây giải thích từng tầng (logic nay nằm trong SQL của các batch spec, không còn Python hardcode).

### 3.1 Bronze → Silver — `silver_enriched_transactions.yaml`
...
### 3.2 Silver → Gold — 3 spec `gold_*.yaml`

Sinh 3 bảng dưới `s3a://data-lake-gold/`: `daily_transaction_summary`, `customer_lifetime_metrics`,
`high_risk_transactions`. Deployer thêm package iceberg tự động cho job cần (chỉ Iceberg).

### 3.3 Silver → Iceberg — `iceberg_silver_enriched.yaml`

`output.format=iceberg` → runner làm CTAS sang `lakehouse.silver.enriched_transactions` (catalog REST).
Deployer tự thêm `--packages iceberg-spark-runtime`. Điểm bẫy: server iceberg-rest trả path scheme
`s3://` mà client HadoopFileIO chỉ có `s3a` → runner đặt `fs.s3.impl=S3AFileSystem` (xem [ADR-0009](../decisions/0009-iceberg-rest-catalog.md)).
