...
1. [x] Viết **JSON Schema** cho chính metadata (`dataplatform/schemas/`) — "meta-metadata".
2. [~] Mã hóa dataset thành YAML như ví dụ §3:
   - [x] 4 dataset OLTP (`customers`, `accounts`, `transactions`, `transfers`)
   - [x] `fraud-alerts` (ngoài danh sách gốc — cần để chứng minh mô hình chịu được dataset khác khuôn)
   - [x] **4 metric** (`metrics.{timeseries,kpi,breakdown,topn}`) — [ADR-0019](../decisions/0019-generate-clickhouse-metric-ddl.md).
...
3. [~] Mã hóa connections — `metadata/connections/*.yaml` là contract hạng nhất ([ADR-0025](../decisions/0025-connection-registry-trino-catalog.md)).
   [x] 3 connection Trino (postgres, clickhouse, iceberg) → sinh catalog. [ ] Còn kafka, s3, es, schema-registry.
4. [x] **Kiểm chứng ngược** (bộ ba nguồn sự thật độc lập) — [ADR-0022](../decisions/0022-reverse-verify-contract-vs-real-schema.md):
   - [x] `verifiers/postgres_schema` — bảng nguồn, so `information_schema` (tên/kiểu/nullable/PK): 4/4 khớp.
   - [x] `verifiers/avro_schema` — Trên dây, so Avro trong Schema Registry (`encoded_as: string` = decimal
     mã hoá string): phần kiểm được khớp (bảng rỗng thì bỏ qua tới khi có data).
   - [x] `verifiers/clickhouse_schema` — bảng đích, so `system.columns` (bắt drift thủ công, tái dùng `_ch_type`): 4/4 khớp.
...
1. Generator sinh các artifact ingestion:
   - [x] 5 file `es-sink-*.json` (topic + `extractKey.field` = PK từ contract) — [ADR-0015](../decisions/0015-metadata-registry-yaml-first.md)
   - [x] `s3-sink-cdc.json` (`topics` gộp từ mọi dataset bật `s3_bronze`)
   - [x] Cấu hình DLQ cho cả 6 connector + bản kê topic cho `dlq-processor` — [ADR-0017](../decisions/0017-dlq-flow-observe-then-park.md)
   - [x] `postgres-connector.json` (`table.include.list`) **và** `04_publication.sql` — sinh từ **cùng
     một nguồn** nên không thể lệch; diệt sprawl #2/#3 ([ADR-0018](../decisions/0018-generate-debezium-and-publication.md)).
   - [x] **Topic manifest + cutover** — sinh `kafka/topics.json` + `kafka/create-topics.sh` từ registry
...
2. [x] **Deployer** áp connector qua Connect REST **idempotent** (`PUT /connectors/{name}/config`), có
...
3. [x] Đối chiếu JSON sinh vs JSON hiện tại (diff = rỗng) — `python -m dataplatform.cli check`, 13/13 khớp.
   [x] **Cắm `check` vào CI** — GitHub Actions `metadata-check.yml` chạy `check` trên mọi PR + push main;
...
1. [~] `flink_runner` (**cấu trúc khai báo hoàn toàn** — [ADR-0023](../decisions/0023-flink-metric-runner-declarative.md)):
   [x] 4 pipeline spec (`metadata/pipelines/stream/*.yaml`) khai window/filter/dimensions/aggregations/rank;
   [x] `generators/flink_sql.py` sinh source DDL (từ cột tham chiếu — diệt sprawl #6, loại cột chết) + sink
   DDL (từ contract metric — diệt nửa Flink #8) + INSERT; [x] runner mỏng `metric_runner.py` thực thi job
   plan; [x] **oracle SQL**: SQL sinh tương đương SQL viết tay lane1; [x] **runtime**: submit RUNNING (mọi
   vertex, kể cả WindowRank cho topn); [x] **parity ground truth**: seed data → cả 4 metric (TUMBLE +
   CUMULATE) khớp tuyệt đối trong Kafka + ClickHouse; [x] **cắt chuyển: xoá `lane1_dashboard.py`**; [x]
   deployer `flink_metrics` (`plan`/`apply`).
2. [x] Fraud (`fraud_runner.py`): **giữ detector là code** (velocity, failed-storm), nhưng source DDL +
...
3. [x] Đối chiếu + cắt chuyển: runner chạy song song (group riêng), parity ground truth → **xoá
...
1. [x] Từ **mỗi metric dataset**, sinh cả 3: `metrics.<m>` (engine/ORDER BY/TTL từ metadata),
...
1. [~] `medallion_runner` (mỏng) + `deployers/spark_batch` (`plan`/`apply` theo thứ tự layer):
   - [x] **Silver** — batch spec `silver_enriched_transactions.yaml` (inputs + SQL dedup+join 3 chiều +
     output schema/partition). **Parity: 72 = 72 rows** với `enrich_transactions.py`; đã xoá job cũ.
   - [x] **Gold** (3 spec) — parity tuyệt đối với `build_gold_layer.py`: daily **339=339**, customer
     **100=100**, high_risk **310=310** (cùng Silver 1072). Đã xoá job cũ.
   - [x] **Iceberg** (`output.format=iceberg` → runner CTAS + catalog REST) — `lakehouse.silver.enriched_transactions`
     1072 rows, 1 snapshot, query được. Đã xoá `silver_to_iceberg.py`. Lôi ra bẫy scheme `s3://` vs `s3a`
     (fix `fs.s3.impl=S3AFileSystem`).
2. [x] Đối chiếu row count Silver + Gold + Iceberg. [ ] Verifier schema output vs `output.columns` (Pha 6/7).
...
1. [x] Sinh `trino/etc/catalog/*.properties` từ **connection registry**.
   - [x] **Connection registry** (`metadata/connections/*.yaml`) — đóng nợ Pha 1 (connection nay là contract
   ...
   - [x] Generator `trino_catalog.py` — sinh 3 catalog, **oracle byte-exact khớp bản viết tay** (`check` 18/18);
     diệt sprawl #13 ([ADR-0025](../decisions/0025-connection-registry-trino-catalog.md)). Secret vẫn `${ENV:...}`.
   - [x] Federation runtime verify **đủ 3 nguồn** (2026-07-20): `postgres.transactions=1046`,
   ...
   - [x] **Encode connection non-Trino** ([ADR-0029](../decisions/0029-encode-connection-non-trino.md)): es_sink/
...
2. [x] **Lineage + catalog** — `generators/lineage.py` sinh `lineage/graph.json` + `LINEAGE.md` thuần từ
   ...
   [x] **OpenMetadata UI** — chọn OpenMetadata (không DataHub, [ADR-0027](../decisions/0027-openmetadata-catalog.md));
   `deployers/openmetadata.py` nạp `graph.json` (**24 table** = 9 dataset + 5 lake + 10 sink ngoài, tag PII,
   **đủ 25 cạnh lineage**), verify qua API. Máy 15.3GB đủ khi tạm dừng stack chính (OM ~3GB). [x] tạo table cho
   đích sink ES/CH/S3 (lineage không còn cụt). [x] **Lineage cột Spark** — sqlglot parse SQL batch (qua CTE/join),
   87 bản ghi lineage cột (17 Flink + 70 Spark), lần được PII `full_name`→`customer_name`→gold ([ADR-0028](../decisions/0028-spark-column-lineage-sqlglot.md)).
3. [x] Trả lời được (qua `LINEAGE.md`): "cột `amount` chảy tới đâu?", "dataset nào chứa PII?", "ai sở hữu?".
...
1. [~] **Orchestration (Airflow/Dagster):** DAG **sinh từ phụ thuộc dataset** (bronze → silver → gold). Có
   ...
   - [x] **`generators/airflow_dag.py`** — sinh `airflow/dags/medallion_batch_dag.py` từ phụ thuộc input/output
   ...
   - [x] **Boot Airflow runtime** (2026-07-20): `apache/airflow:2.10.4` standalone, DAG load qua DagBag thật
...
2. [~] **CI/CD "plan → apply"**: PR sửa contract → CI **validate** (JSON Schema) + **compatibility gate**
   ...
   - [x] **`cli plan`** — render hệ quả artifact khi merge (mới/sửa vs base ref).
   - [x] **`cli compat`** — gate BACKWARD, chặn breaking change ở PR ([ADR-0030](../decisions/0030-ci-plan-compat-gate.md));
...
3. [~] **Migration có version** cho DDL (ClickHouse/Iceberg) — không init-once.
   - [x] **ClickHouse**: runner `clickhouse_migrate` + `migrations/clickhouse/NNNN_*.sql`, sổ cái
...
4. [x] **Data quality gate:** `verifiers/quality.py` + `metadata/quality/*.yaml` — not_null/unique tự suy
...
5. [x] **RBAC & audit:** `.github/CODEOWNERS` theo vùng metadata + branch protection + `owner` trong contract
   + audit Git/lineage ([ADR-0035](../decisions/0035-rbac-codeowners.md)).

**Đã thêm:** [x] **Rollback** deployer từ git ref ([ADR-0034](../decisions/0034-rollback-via-git-ref.md));
[x] **Migration Iceberg** = native evolution (không runner — [ADR-0036](../decisions/0036-iceberg-native-evolution.md)).
...
1. [x] Cutover diễn ra dần qua Pha 2–7 (chứng minh byte-exact → xoá bản viết tay). Audit cuối: chỉ còn
   `trino/iceberg.properties.bak` di sản → đã xoá. Không còn file viết tay song song bản sinh.
2. [x] **Runbook** [`docs/guide/runbook.md`](../guide/runbook.md): thêm cột/bảng/metric/connection, breaking
   change, migration, rollback, backfill, quality — quy về "sửa metadata + chạy" + tham chiếu lệnh + gotchas.
3. [x] Chốt: **một nơi để sửa — `metadata/`**. 19 artifact sinh, `check` 19/19, sprawl #1–#13 đóng.
...
1. [x] Thêm một **cột** = sửa 1 contract → CI plan/apply → mọi engine cập nhật, không sửa file nào khác.
2. [x] Thêm một **bảng/metric** = thêm 1 file trong `metadata/`, không viết code engine mới.
3. [x] Không còn artifact viết tay song song (JSON connector, DDL init, `lane1_*.py`, job Spark cứng).
4. [x] Mọi thay đổi qua **PR có review + compatibility gate + quality gate + rollback**.
5. [x] **Lineage & ownership** tra cứu được tự động cho mọi dataset.
6. [x] **Không secret nào** ở dạng plaintext ngoài secret manager; CI có quét secret.
7. [x] Có **runbook** để người mới thao tác chỉ bằng cách sửa metadata.
