...
Lý do (nay ở nhánh iceberg của `spark/jobs/medallion_runner.py`):
...
> **Bẫy khi tổng quát hoá (2026-07-19):** server iceberg-rest trả path scheme `s3://` (nó dùng S3FileIO
> riêng), nhưng client HadoopFileIO chỉ có `s3a`. Fix: `fs.s3.impl = S3AFileSystem` để `s3://` đi qua S3A
> (đọc config `fs.s3a.*` sẵn có). Xem [ADR-0024](0024-spark-medallion-runner-sql.md).
